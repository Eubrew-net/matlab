function status = mat2sheets_jls(spreadsheetID, sheetName, sheetpos, d)
% mat2sheets_jls: a modification  of mat2sheets that uses the
% sheet name instead of the sheet ID as input parameter.
%
% The original documentation of the function follows.
%  
% 20190521 JLS
%
...
%--------------------------------------------------------------------------
function sheetID = getSheetID(spreadsheetID, sheetName)
% a very unoptimized modification of getSheetMetaData to get the
% sheet name from the sheet ID
% JLS 20190521
...
if success
    ...
    jdat = loadjson(j, opt);
    sIds = nan(size(jdat.sheets));
    sNames = strings(size(jdat.sheets));
    if iscell(jdat.sheets(1)) % test for some weird bug
        for i = 1:numel(jdat.sheets) 
            jd = jdat.sheets(i); 
            jdc = cell2mat(jd); 
            sIds(i) = jdc.properties.sheetId;
            sNames(i) = jdc.properties.title; 
        end 

        wheresheet = find(sNames==sheetName);
        ...
        %    nRows = jdc.properties.gridProperties.rowCount; 
        %    nCols = jdc.properties.gridProperties.columnCount; 
            sheetID_num = jdc.properties.sheetId; 
        else 
            disp('Can not find the sheet!'); 
        end 
    end
    sheetID=num2str(sheetID_num);
end