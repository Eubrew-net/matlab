...
if nargin==0 % callback or command line update requested
   ...
   rtick=round(linspace(rmin,rmax,ntick)); % right side tick marks
   re=max(abs(rmm));
...
rtick=round(linspace(rlims(1),rlims(2),ntick)); % right side tick marks
if abs(re)>3 % scale right side tick marks
   rtick=rtick/(10^re);
end
rmm=[rtick(1) rtick(ntick)];rx=max(abs(rmm));
dig=min(ceil(abs(log10(abs(rmm(2)-rmm(1))/rx)))+2,4);
