function [x,r,ab,rp,data,osc_out,osc_smooth,outliers]=ratio_month_ozone(a,b,n_min,name_a,name_b,varargin)
...
arg.addParamValue('plot_flag', 0, @(x)(x==1 || x==0)); % Por defecto 1 (no ploteo de individuales)
...
[mx,sx,outliers.rp,idxp]=outliers_bp(rp(:,2));
...
try 
   osc_ranges=[300,550,850,1250,1500,1800];
...
% rp(find(rp(:,2)<-1.4),:)=NaN;
    ...
    try     
     %P0=gscatter(osc,rp(:,2),diaj(data(:,1)),[],'.');   
     %hold on
     P=gscatter(osc,rp(:,2),month(data(:,1)),[],'o');
    ...
    set(gca,'Xlim',OSC_lim,'Ylim',[-3,3]);
     ...
     title(sprintf('(%s - %s) / %s.  Grouped by Month',name_a,name_b,name_b))
     box on;
    ...
    f=figure; 
    set(f,'Tag','RATIO_SZA'); 
    j=data(:,1)-fix(data(:,1))>=0.5;  %am/pm
    %hold on;
    %plot(sza(j),rp(j,2),'o');
    %j=find(data(:,1)-fix(data(:,1))<0.5);
    %plot(sza(j),rp(j,2),'+');
    gscatter(sza,rp(:,2),{month(data(:,1)),j},[],'o+',5,'off'); 
    set(findobj(gcf,'Tag','legend'),'Location','NorthEast');
    ...
    try
     ...
     l=legend(gca,filter_name{unique(data(:,9))+1},'Orientation','Horizontal','Location','NorthEast');
     set(l,'FontSize',12);
    ...
    set(gca,'XTick',unique(data(~isnan(data(:,9)),9))','XTickLabel',x_); 
    %%
    %figure by temp 
    f=figure;    set(f,'Tag','RATIO_TEMP_INST');    
       P=gscatter(osc,rp(:,2),fix(data(:,8)/7.5)*7.5,'','o',5);  %,data(:,end-data_l+1)],'','+o');
       set(gca,'Ylim',[-3 3]); grid;
       set(findobj(gca,'Type','Line'),'MarkerSize',5);
       set(findobj(gcf,'Tag','legend'),'Orientation','Horizontal','Location','NorthEast');
    opts.selected.Marker='x';     opts.selected.Color='k';
