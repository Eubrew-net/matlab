...
%           doesn't allow changing the lineColor property of a histc bar graph.)
            ...
            if ~smoothFlag
                stairs(x{k},n{k},'k','linewidth',linewidth,'Marker','None');
                plot([x{k}(1) x{k}(1)],[0 n{k}(1)],'color','k','linewidth',linewidth,'Marker','None');
...
%       the main line
        if ~topFlag || x(k)>=0  %also plot the bottom half.
            plot([x(k)+ex x(k)],[y(k) y(k)],'color',lineStyleOrder{k},'linewidth',linewidth,'Marker','None');
    %       the hat     
            plot([x(k)+ex x(k)+ex],[y(k)+esy y(k)-esy],'color',lineStyleOrder{k},'linewidth',linewidth,'Marker','None');
        end
        if ~topFlag || x(k)<0  %also plot the bottom half.
            plot([x(k) x(k)-ex],[y(k) y(k)],'color',lineStyleOrder{k},'linewidth',linewidth,'Marker','None');
            plot([x(k)-ex x(k)-ex],[y(k)+esy y(k)-esy],'color',lineStyleOrder{k},'linewidth',linewidth,'Marker','None');
            %rest?
        end
...
%         the main line
        if ~topFlag || y(k)>=0 %also plot the bottom half.
            plot([x(k) x(k)],[y(k)+ey y(k)],'color',lineStyleOrder{k},'linewidth',linewidth,'Marker','None');
    %       the hat
            plot([x(k)+esx x(k)-esx],[y(k)+ey y(k)+ey],'color',lineStyleOrder{k},'linewidth',linewidth,'Marker','None');
        end
        if ~topFlag || y(k)<0 %also plot the bottom half.
            plot([x(k) x(k)],[y(k) y(k)-ey],'color',lineStyleOrder{k},'linewidth',linewidth,'Marker','None');
            plot([x(k)+esx x(k)-esx],[y(k)-ey y(k)-ey],'color',lineStyleOrder{k},'linewidth',linewidth,'Marker','None');
        end
    end
end
...
if pointsFlag
    for k=1:num2Plot
        plot(x(k),y(k),'-','markersize',8,'color',lineStyleOrder{k},'MarkerFaceColor',lineStyleOrder{k});
    end
end
...
if outlierFlag % but only if you want to
    ...
    if ~isempty(yy)
        ...
        if ~isempty(xx)
            if horizontalFlag
                hReg2(6) = plot(xx,yy,'-','linewidth',linewidth,'color',wisColor);
            else
                 hReg2(6) = plot(yy,xx,'-','linewidth',linewidth,'color',wisColor);
            end
        end
    end
end
