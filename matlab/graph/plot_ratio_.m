...
%%
osc_ranges={'<600','600-1000','>1000'};

aux=NaN*ones(size(dat,1),size(dat,2)+1);
aux(:,1:end-1)=dat;
      %m_ref  %O3_inst
osc_s=dat(:,4).*dat(:,5);
aux(osc_s<600,end)=1; aux((osc_s>=600 & osc_s<1000),end)=2;
%aux((osc_s>=550 & osc_s<850),end)=3; aux((osc_s>=850 & osc_s<1250),end)=4;
aux(osc_s>=1000,end)=3;
...
set(g,'Marker','.','MarkerSize',5); set(gca,'Ylim',[-3 3]);  grid; box on;  
legend(osc_ranges,'location','West','FontSize',8,'FontWeight','Bold'); 
    
hline([-0.5 0.5],'k--'); hline([-1 0 1],'k-'); 
text(11/13,10/11,[inst,' - ',ref,' / ',ref],'Units','Normalized','BackGroundColor','w','FontWeight','Bold');
