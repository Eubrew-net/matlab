...
%$Revision: 1.10.2.3 $   $Date: 2006/11/08 17:47:31 $ 
...
if (ExcelDateNumber <= 0)
     error('finance:x2mdate:invalidInputs','Inputs must be positive!')
end
...
if ((size(Convention) ~= size(ExcelDateNumber)) & (max(size(Convention)) ~= 1))
     error('finance:x2mdate:invalidConventionFlagSize','Convention flag must be the same size as date number vector or scalar!')
elseif (length(Convention(:)) == 1)
     Convention = Convention * ones(size(ExcelDateNumber));
end

if (any(Convention ~= 0 & Convention ~= 1))
     error('finance:x2mdate:invalidConventionFlag','Invalid convention flag specified!')
end
