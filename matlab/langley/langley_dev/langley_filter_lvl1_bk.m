...
arg.addParamValue('F_corr', [],  @(x)iscell(x) || @(x)isfloat(x));         % default no filter corr
...
if ~isempty(arg.Results.F_corr)
    ...
    if iscell(f_corr)
       day_1=cellfun(@(x) x(end,2),f_corr); day_2=cellfun(@(x) unique(fix(x(:,1))),data);
       a=ismember(day_1,day_2); f_corr(find(a==0))=[];
       
       f_corr=cellfun(@(x,y) cat(2,[0,0,x(29,2),x(30,2),x(31,2),0]),f_corr,'UniformOutput',false);    
    end
    
    for dd=1:size(data,1)
        MS9_n=data{dd}(:,[10 end]);
        for filtro=1:6
            idx_flt=MS9_n(:,1)==64*(filtro-1);
            MS9_n(idx_flt,2)=matadd(MS9_n(idx_flt,end),-f_corr{dd}(filtro));
        end   
        data{dd}(:,end)=MS9_n(:,2);
    end
end
