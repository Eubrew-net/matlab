function  [A,ETC,SL_B,SL_R,F_corr,SL_corr_flag,cfg]=read_config(config,file_setup,sl_s) 
% Updated:
...
for i=[Cal.n_inst,Cal.n_ref]
    ...
    if ~isempty(a)               
       ...
       if isempty(ext)% trabajamos con una �nica config. -> first = bfile
          ...
          SL_corr_flag.old(idx,i+1)=1; % es irrelevante en este caso. La correcci�n vendr� dada por test_recalculation.m
          for f=1:6
       ...
       if strcmp(ext,'.cfg')
          ...
          SL_corr_flag.new(idx,i+1)=1; % es irrelevante en este caso. La correcci�n vendr� dada por test_recalculation.m
          try
             for f=1:6
