...
function [sl_s,slavg_prevplot,oulier,R6]=sl_report(idx_inst,sl,brw_name,date_range,fplot)
...
slavg(:,2)=[];
slavg_prevplot=slavg;
% sl_s contiene los valores suavizados (media móvil de 7 días). Entonces,
% ¿por que el if?. Aunque tengamos menos de 5 días podemos seguir
% calculando valores suavizados. Por eso he comentado (si no, cuando size(slavg,1)<5 entonces no tendríamos una media
% móvil, sino que seguiriamos teniando las medias diarias, slavg. Además, con sl_s(:,1)=sl_s(:,1)+.5; los dos ploteos, 
...
if nargin==5
   if fplot==1
    ...
    slaux=sl_s;
    slavg(:,1)=diaj(slavg(:,1))+.5;% sumando .5 aqui colocamos los puntos en 205.5 (si el dia juliano es 205, por ejemplo, y queda chachi) 
    p1=errorbard(slavg(:,1:3),'s');% R6 daily mean
    set(p1,'MarkerEdgeColor','k','color','g');
    %set(p1(1),'Linewidth',3);
    hold on
    %p2=stairs(diaj(slavg(:,1)),slavg(:,2),'o-','LineWidth',2);
    p2=plot(diaj(slaux(:,1))+.5,slaux(:,2),'o-','LineWidth',1); % R6 smooth 7
    p3=plot(diaj2(sls(:,1)),sls(:,22),'.','Markersize',8); % R6 measures
    legend([p1(1),p2,p3],'R6 daily mean ','R6 smooth 7','R6 measures','Location','Best');
    ...
    p1=errorbard(slavg(:,[1,4,5]),'s');
    set(p1,'MarkerEdgeColor','k','color','g');
    %set(p1(1),'Linewidth',2);
    hold on
    p2=plot(diaj(slaux(:,1))+.5,slaux(:,4),'o-','LineWidth',1);
    p3=plot(diaj2(sls(:,1)),sls(:,21),'.','Markersize',8);
    legend([p1(1),p2,p3],'R5 daily mean ','R5 smooth 7','R5 measures','Location','Best');
    ...
    p1=errorbard(slavg(:,[1,8:9]),'s');
    set(p1,'MarkerEdgeColor','k','color','g');
    %set(p1(1),'Linewidth',3);
    hold on
    p2=plot(diaj(slaux(:,1))+.5,slaux(:,8),'o-','LineWidth',1);
    p3=plot(diaj2(sls(:,1)),sls(:,23),'.','Markersize',8);
    %legend([p1(1),p2,p3],'I_5 smooth 7','I_5 daily mean ','I_5 meas');
    legend([p1(1),p2,p3],'I_5 daily mean ','I_5 smooth 7','I_5 meas','Location','Best');
