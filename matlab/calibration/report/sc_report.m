    ...
    for jj=1:length(dias_)
        ...
        if ~isempty(scraw_)
            for ii=1:size(scavg_,1),
                ...
                if isnan(a(ii,10)) a(ii,10)=v(1); end  % step
                if isnan(a(ii,11)) a(ii,11)=v(2); end  % ozone
                if isnan(a(ii, 8)) a(ii,8)=nanmean(sc_(:,8)); end %
                if isnan(a(ii, 9)) a(ii,9)=nanmean(sc_(:,6)); end %airmass
                sca=a(ii,:);
