...
%
% 
function [NTC,tabla_regress]=temp_coeff_report_dev(config_file,sl,config,daterange,outlier_flag)
...
%  for ii=1:7
%      subplot(4,2,ii)
%      mmplotyy_temp(Fr_dep{ii}(:,1),Fr_dep{ii}(:,end),'bx',Fr_dep{ii}(:,end-1),'rx');
%      datetick;
%  end
...
ylabel('Counts');
xlabel('date');
text(repmat(sls(1,1)+.5,5,1),nanmean(Fr(:,3:end-2)),...
...
for ii=0:5
 ...
 plot(Fr(out,2),Fr(out,3+ii),'x','MarkerSize',14);
 [h]=gscatter(Fr(:,2),Fr(:,3+ii),{year(Fr(:,1)),ceil(diaj2(Fr(:,1))/nrep)*nrep},c,[],10);
