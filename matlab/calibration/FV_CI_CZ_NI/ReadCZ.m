...
function [GRT FH]= ReadCZ(filename)
...
s=fileread(filename);
[mat idx1] = regexp(s, 'scan start time = ', 'match', 'start');
[mat idx2] = regexp(s, 'Dark Count =  ', 'match', 'start');
[mat idx3] = regexp(s, 'Counts/Second', 'match', 'start');
[mat idx4] = regexp(s, 'scan finished at ', 'match', 'start');
...
a= size('scan start time = ',2);
b= size('Dark Count =  ',2);
c= size('Counts/Second ',2);
d= size('scan finished at ',2);
e= size('end',2);
