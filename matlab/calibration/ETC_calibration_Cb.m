%function [ETC_NEW,o3c]=ETC_calibration(file_setup,summary,instrumento,refernecia,tsync,oscmax,szasync,C_DAYS)
%% Calibration
...
%         % ETC determination
         ...
         %%two point by filter
         figure;gscatter(ozone_slant(j),o3_c(j,21),o3_c(j,18));hold on;plot(ozone_slant(j),o3_c(j,21),'kx');[rk,l]=rline;title(num2str(l));
         ylabel('MS9  corrected')
         figure;gscatter(ozone_slant(j),o3_c(j,22),o3_c(j,18));hold on;plot(ozone_slant(j),o3_c(j,22),'kx');[rk,l]=rline;title(num2str(l))
         ylabel('MS9 un corrected')
         
         %%
         filter=unique(o3_c(j,18));dummy_var=[];for i=1:length(filter),   dummy_var(j,i)=(o3_c(j,18)==filter(i)); end
         [a,b,c,d]=regress(o3_c(j,22),[ozone_slant,dummy_var,ones(size(ozone_slant))]);
         sum(dummy_var)
         disp(fix([a,b]));
         
         % standard regression
         [a,b,c,d]=regress(o3_c(j,22),[ozone_slant,ones(size(ozone_slant))]);
         sum(dummy_var)
         disp(fix([a,b]));
         
         %regresion by filter
         [a,b,c,d]=regress(o3_c(j,22),[ozone_slant,dummy_var]);
         sum(dummy_var)
         disp(fix([a,b]));
