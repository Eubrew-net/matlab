function [cal,summary,summary_old,A1]=summary_reprocess(file_setup,ninst,ozone_ds,ozone_sum,config,sl_s,flag_sl,sl_ref)
...
% 12  ozone sl -> config 1 +   sl correction 
    ...
    if isstruct(config)
      ...
      cfg=cell2mat(config{i}');     
      A_new=[cfg(end,2:3:end);cfg(8,2:3:end)]';
      A_old=[cfg(end,1:3:end);cfg(8,1:3:end)]';
      [a,b]=findm(ozone1(:,1),A_new(:,1),.8);
      A.new=NaN*ozone1(:,1);A.new(a,1)=A_new(b,2);      
      A.old=NaN*ozone1(:,1);A.old(a,1)=A_old(b,2);      
    end   
 ...
 if ~isempty(ozone1) 
    ...
    if flag_sl
        ...
        ozo_c=ozone1(:,15)+(SL_NEW_REF(i)-R6)./(10*A.new.*ozone1(:,5));
        %ozo_c=ozone1(:,15);
        ozo_o=ozone1(:,8)+(SL_OLD_REF(i)-R6)./(10*A.old.*ozone1(:,5));
