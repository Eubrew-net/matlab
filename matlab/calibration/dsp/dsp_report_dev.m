...
[Jbad,sbad]=find(abs(fwl)>0.125);
%sbad=unique(sbad);
%Len=sum(find(fwl),2);
Len=sum(fwl(:,unique(sbad))~=0);
Res=max(abs(fwl(:,unique(sbad))));
Res=Res & (Len>3);
while ~isempty(Jbad) && any(Res)
    %ns=1;
    %while ns<=length(Res) %slit
        slits=unique(sbad);
        i=slits(1);
        fwll=fwl(:,i);
        jbad=find(abs(fwll)>0.125);
        [a,b]=sort(abs(wl(jbad)-lamda_nominal(i)),'descend'); % Bug estaba 2
        jbad=jbad(b);
        len=size(find(fwll),1);
           
        %Para la slit 
        while ~isempty(jbad) && len>3
                    
             dsp(jbad(1),i)=0;
             disp(sprintf('eliminamos la linea %d slit %d %f %f',jbad(1),i,wl(jbad(1)),fwll(jbad(1))));                   
             [fwl,fstps,pwl,pstps]=normaldsp(wl,dsp);  % quad for every slit
             jbad=find(abs(fwl(:,i))>0.125)
             [a,b]=sort(abs(wl(jbad)-lamda_nominal(i)),'descend'); % Bug estaba 2
             jbad=jbad(b);
             len=size(find(fwl(:,i)),1);
        end                 
...
quad_report=num2cell(res(jcal,1:5));
...
% modificado el 05/11/2009
 ...
 step_report=num2cell(detail(1:5,1:6,jcal+ii));
 %  end -2 introducimos daumont
 quad_res=num2cell(res(jcal+ii,1:5));
...
res=cat(3,[res(jcal-3:jcal+3,:);res(end-1:end,:)],[res2(jcal-3:jcal+3,:);res2(end-1:end,:)]);
detail=cat(4,detail(:,:,[jcal-3:jcal+3,end-1:end]),detail2(:,:,[jcal-3:jcal+3,end-1:end]));
