function [dsp_quad dsp_cubic salida]=read_dsp_report(dsp_dir,varargin)
        ...
        if ~arg.Results.process
            try
              ...
              salida{idx}=dsp_sum;
               ...
               fprintf('%s\n',exception.message);  
               try
                  [wv_quad wv_cubic output ]=no_hay_mat(dsp_dir,ldsp,indx,{arg.Results.inst,Cal.brw_str},Cal.brw_config_files);
                  ...
                  salida{idx}=output;
                   ...
                   wv_matrix_quad=cat(1,wv_matrix_quad,wv_quad); 
                   wv_matrix_cubic=cat(1,wv_matrix_cubic,wv_cubic);
                   salida{idx}.salida.CUBIC.resumen=NaN*ones(1,15);
                   %salida{idx}.salida.CUBIC=NaN*ones(1,15)
               end
...
% --------------------------------------------------------------------------------------
function [wv_matrix_quad wv_matrix_cubic salida]=no_hay_mat(path_dsp,ldsp,indx,inst_info,brw_config_files) 
