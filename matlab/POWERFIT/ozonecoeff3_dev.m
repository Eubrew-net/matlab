...
at2=load('atlas3_br.dat'); % wl in vacuum (correct by about 0.1nm)
at2(:,1)=wlrefrac(at2(:,1))*10;  % change to air
at2(:,2)=at2(:,2)/1000;
% real spectra
at3=load('spectra');
at3=at3.spectra;
...
for opos=oposrange;
    ...
    for i=1:6,  % slits 0 to 5
        ...
        if sum(ind)>1,
            ...
            ETS=scan_join(at2,at3);
            buf=brslit(fwhmwl(i),ETS(:,1)-thiswl(i));
            if ~isempty(uvr)
                uvr_i=interp1(uvr(:,1),uvr(:,2),ETS(:,1));
                at3_r=ETS(:,2).*uvr_i;
                at2_r=ETS(:,3).*uvr_i;
            ...
            I0(i)=nansum(buf(:,2).*ETS(:,3))/nansum(buf(:,2));
            F0(i)=nansum(buf(:,2).*ETS(:,2))/nansum(buf(:,2));
        else
            o3coeff(i)=nan;
            raycoeff(i)=nan;
            thiswl(i)=nan;
    ...
    F0Coeff=((log10(F0(2+1)))-0.5*(log10(F0(3+1)))-2.2*(log10(F0(4+1)))+1.7*(log10(F0(5+1))))*1e4;
    ...
    resumen_=[opos-cal_ozonepos,O3Coeff,RAYCoeff*1e4,So2coeff,O34So2cc,...
        -I0Coeff,O3Daumont,-F0Coeff];
    resumen=[resumen;resumen_];
    salida_m(:,:,jj)=[thiswl;fwhmwl;o3coeff;raycoeff;so2coeff;I0;F0];
    ...
    if abs(opos-mic_ozonepos)<5
       if abs(opos-mic_ozonepos)<2 % preesentamos +/- 1 paso
          disp(s1)
          disp(s2)
          disp(s3)
          disp(s3c)
...
for opos=[mic_ozonepos,umk_step];
    ...
    resumen_=[opos-cal_ozonepos,O3Coeff,RAYCoeff*1e4,So2coeff,O34So2cc,-I0Coeff,O3Daumont,-NaN];
    resumen=[resumen;resumen_];
    salida_aux=[thiswl;fwhmwl;o3coeff;raycoeff;so2coeff;I0;NaN*I0];
    salida_m(:,:,end+1)=salida_aux;
