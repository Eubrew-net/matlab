...
date_initial=[datenum(2005,1,1) now];
nlamp={'026','034','039','646','650','856','957','958','1004','1005','1080','1081','1083'};% 

% Año 2005: '646','650','1080','1081','1083'
% Año 2006: '646','650','957','958','1080','1081'
% Año 2007: '646','650','1080','1081'
% Año 2008: '646','650','1080','1081','1004'
% Año 2009: '646','650','856','1004','1005','1083'
% Año 2010: '856','1080'

% Año 2008 y 2009 new standar: '103','857','856','1083'
% Todo: '026','034','039','646','650','856','957','958','1004','1005','1080','1081','1083'
...
ref_5=r_50w(1,2:end);
H=mmplotyy(r_50w(:,1),median(matdiv(r_50w(:,2:end),ref_5)')*1.05,'o',temp_r50(:,2),':^',[10 40]);
...
hold on; 
plot(gca,r_50w(:,1),smooth(r_50w(:,1),median(matdiv(r_50w(:,2:end),ref_5)')*1.05,7),'-g^','Linewidth',2.5)
...
periodos={
%           periodo1: 1/Mayo/2005 a 5/Octubre/2006 (Antes de IOS06)
            [datenum(2005,1,1) datenum(2006,10,5)]
%           periodo2: 6/Octubre/2006 a 25/Septiembre/2007 (between IOS's)            
            [datenum(2006,10,6) datenum(2007,09,25)]
%           periodo3: (from IOS07 to IOS09)
            [datenum(2007,09,26) datenum(2009,08,28)]           
%           periodo4: (from IOS09 to now. Pendiente de calibración en Lab.)
            [datenum(2009,08,29) datejuli(2010,100)] 
%           periodo5: (from IOS09 to now. Pendiente de calibración en Lab.)
            [datejuli(2010,101) now]%  datenum(2009,08,29)        
            
         };
% %           periodo5: from IOS10 to now, last update )
%             [datenum(2010,09,20) now]           
     
events_line=cell2mat(periodos)';  events_label={'IOS06','IOS07','IOS09','Chg?','IOS10'};  
...
for p=1:length(periodos)
    ...
    elseif p==4 % cogemos la media
       intpol{p}=[r_kw(indx,1),repmat(mean(r_kw(indx,2:end)),length(indx),1)];        % Sólo lamps    
       extpol{p}=[extpol_per,repmat(mean(r_kw(indx,2:end)),length(extpol_per),1)];    % Periodos        
    elseif p==5 % Interpolaciòn lineal (ver grafico, si sale ;))
       for l=1:24
%            poly_coeff=pchip(r_kw(indx,1)',r_kw(indx,l+1)');
%            intpol{p}(:,l+1)=ppval(poly_coeff,r_kw(indx,1)');        % Sólo lamps
%            extpol{p}(:,l+1)=ppval(poly_coeff,extpol_per);           % Periodos
    ...
    elseif p==3 % Interpolaciòn lineal (ver grafico, si sale ;))
       for l=1:24
%            poly_coeff=pchip(r_kw(indx,1)',r_kw(indx,l+1)');
%            intpol{p}(:,l+1)=ppval(poly_coeff,r_kw(indx,1)');        % Sólo lamps
%            extpol{p}(:,l+1)=ppval(poly_coeff,extpol_per);           % Periodos
...
figure
H=plot(r_50w(:,1),median(matdiv(r_50w(:,2:end),ref_5)')*1.0550,'o'); 
set(H,'LineWidth',2);
hold on; 
plot(gca,r_50w(:,1),smooth(r_50w(:,1),median(matdiv(r_50w(:,2:end),ref_5)'),7)*1.0550,'-g^','Linewidth',2.5)
H=plot(r_kw(:,1),median(matdiv(r_kw(:,2:end),ref_)'),'--sk');
set(H,'LineWidth',2);
vl=vline(events_line(2,1:end-1),'-k',events_label(1:end-1)); set(vl,'Linewidth',1.5); 
vl_=vline([datejuli(2005,261) datejuli(2010,263)],'--k',{'IOS05','IOS10'}); set(vl_,'Linewidth',1.5); 
...
vl=vline(events_line(2,1:end-1),'-r',events_label(1:end-1)); set(vl,'Linewidth',1.5); 
vl_=vline([datejuli(2005,261) datejuli(2010,263)],'--r',{'IOS05','IOS10'}); set(vl_,'Linewidth',1.5); 
...
title('Residuos (%)','FontWeight','Bold');
vl=vline(events_line(2,1:end-1),'-r'); set(vl,'Linewidth',1.5); 
vl_=vline([datejuli(2005,261) datejuli(2010,263)],'--r'); set(vl_,'Linewidth',1.5); 
datetick('x',12,'keeplimits','keepticks'); grid
