% opciones_month.outputDir=fullfile('QL183','html',sprintf('month%02d',month(now))); opciones_month.showCode=false;
% publish('ProcLampsCal_183_juanjo',opciones_month)
...
date_initial=[datenum(2011,01,01)];
nlamp={'026','034','039', '856','1080'};
...
dirout='.\183\procesado'; %directorio al que se llevan las figuras creadas
dirirr='.\certificados'; %directorio en el que se encuentran los ficheros de irradiancia absoluta de las lámparas
s_ref='.\QL183\UVR\uvr36310.183'; % respuesta usada como referencia
...
% path de Matlab, calcula también las respuestas y saca dos (tres si pasamos ref) gráficas por
% cada lámpara.
cuenta0=1;
for i=1:length(nlamp);
...
ref_ind=find(diaj(r_50w(:,1))==131); ref_5=r_50w(ref_ind,2:end);
...
ref_=r_kw(1,2:end);% Cambia la respuesta espectral
