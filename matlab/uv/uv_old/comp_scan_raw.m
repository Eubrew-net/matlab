function [ratio,uv,time]=comp_scan(uv1,uv2)
...
[c,i1,i2]=intersect(round(uv1.time(1,:)*3/10),round(uv2.time(2,:)*3/10)) % tiempos comunes (al minuto)
...
for i=1:count
   ...
   y=[uv1.raw(l1,i1(i)),uv2.raw(l2,i2(i))];
   j=find(x<2950);
   x(j,:)=NaN;
   y(j,:)=NaN;
   y2=100*((y(:,1)-y(:,2))./y(:,2));
   ...
   l1=legend(a,sprintf('%8s  %03s',datestr(uv1.time(1,i1(i))/60/24,15),leg1),...
            sprintf('%8s  %3s', datestr(uv2.time(1,i2(i))/60/24,15),leg2),2);
   set(l1,'FontWeight','demi');
   l2=legend(b,sprintf('ratio (%%) [%03s-%03s/%03s]  ',leg1,leg2,leg2),4);
   set(l2,'FontWeight','demi');
   
   ht=title(sprintf(' Intercomparison plot (Raw) %03s vs %03s %02d/%02d',...
   ...
   print('-dpsc','-append',['comp_raw_',leg1,'_',leg2]) ;
   if i==13
       saveas(gcf,['comp_12h_raw_',leg1,'_',leg2],'fig')
   else   
        close;
   end
end
...
axis([-Inf,Inf,-25,25]);
 ht=title(sprintf(' Intercomparison (Raw) ratio %03s vs %03s %02d/%02d',...
       leg1,leg2,uv1.date(1,1),uv1.date(2,1)));
   set(ht,'Fontweight','bold');
...
hold off;
print('-dpsc',['compr_raw_',leg1,'_',leg2]) ;
saveas(gcf,['compr_raw_',leg1,'_',leg2],'fig')    
end
end
