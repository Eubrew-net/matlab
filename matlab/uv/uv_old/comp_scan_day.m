function [ratio,uv,time,lamda]=comp_scan(uv1,uv2,fplot)
...
if ~isempty(uv1.time) & ~ isempty(uv2.time)
    ...
    date1=datenum(2000+uv1.date(1,:)',1,0)+uv1.date(2,:)'+uv1.time(1,:)'/60/24;
    date2=datenum(2000+uv2.date(1,:)',1,0)+uv2.date(2,:)'+uv2.time(1,:)'/60/24;

    %[c,i1,i2]=intersect(round(uv1.time(1,:)),round(uv2.time(2,:))) % tiempos comunes (al minuto)
    [c,i1,i2]=intersect(fix(date1*60*24/2)*2,fix(date2*60*24/2)*2); % tiempos comunes (al 2 minuto)
    [datestr(date2(i2)),repmat(' ',length(c)),datestr(date1(i1))]
...
%        close all;
    ...
    if(count>0)
        ...
        hold off;
        %print('-dpsc',['Xcompr_',fecha,'_',leg1,'_',leg2]) ;
        orient portrait;
...
%         lamda_=lamda(:,1);
%         g=lamda_;
%         lamda_grp=[2800,2950,3000,3100,3200,3300,3400,3500,3700]
