function [ratio,uv,time]=comp_scan_ss(uv1,uv2)

% eleccion de candidatos
ratio=[];uv=[];time=[];l=[];
c1=find(isletter(uv1.type(2,:))); % scanes tipo ua sincronizados
c2=find(isletter(uv2.type(2,:))); % scanes tipo ua sincronizados
...
%Longitud de onda de sincronismo  --->revisar presupone que son iguales
     ...
     time1=uv1.time(unique(l_1),unique(j1));
     time2=uv2.time(unique(l_2),unique(j2));
...
% buscamos las comunes en la longitud de onda de sincronismo
[c,i1,i2]=intersect(round(time1),round(time2)) % tiempos comunes (al minuto)
...
for i=1:count
   ...
   y=[uv1.ss(l1,i1(i)),uv2.ss(l2,i2(i))];
   j=find(x<2950);
   x(j,:)=NaN;
   y(j,:)=NaN;
   y2=100*((y(:,1)-y(:,2))./y(:,2));
