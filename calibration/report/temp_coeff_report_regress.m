...
for i=1:5 % estimamos tendencia
    p{i}=regress(Fr(:,i+2),[ones(size(Fr,1),1) Fr(:,1)]);
end

for i=1:5
    model(:,i)=p{i}(1) + p{i}(2)*Fr(:,1);
end
% restamos tendencia
Fr_des=(Fr(:,3:end-2)-model(:,:))+repmat(nanmean(Fr(:,3:end-2)),size(Fr,1),1);
