...
%                                                                                                
% Juanjo 01/04/2011: Eliminados los ploteos de R5 (old y new). Se mantiene sólo R6, por claridad 
% 
% Juanjo 05/05/2012: Corregido bug en legenda de OLDvsNEW. La incoherencia que a veces se daba entre la info
%                    de la línea de comando y la legenda del gráfico se debe a que la leyenda se refería a un
%                    ajuste sobre los sumarios recalculados. Se usa el ajuste calculado al principio de todo, usando
%                    las medidas individuales: (robustfit, líneas 262->TC=0, Fr; 267->TC orig, F_orig; 320->TC new, FN). Output ajuste 
% 
% Ejemplo:
...
% date, temp, "ozone" counts/sc (with TC=0), R5 and R6 (TC=0)
F_orig=[aux(:,1),aux(:,4),NaN*aux(:,20:24),aux(:,29:30)];
% R5 and R6 from bfile (7xsummary, TC from bheader)
...
A=zeros(2,7);
for i=1:7 
    [b_tc0(:,i),stats]=robustfit(Fr(:,2),Fr(:,i+2));
    stats=stats.se;
    ajuste.cero(i,:)=[b_tc0(1,i)' b_tc0(2,i)' stats(1,:)' stats(2,:)'];
    
    if any(i==[1,5,6,7])
     [b_orig(:,i),stats]=robustfit(F_orig(:,2),F_orig(:,i+2));
     stats=stats.se;
     ajuste.orig(i,:)=[b_orig(1,i)' b_orig(2,i)' stats(1,:)' stats(2,:)'];
    end    
        
end

% a partir de ahora NTC siempre se obtendra a partir de las cuentas con TC=0
tc=b_tc0(2,1:5); otc=TC_B; % TC wich where used for ratios calculations
...
b=zeros(2,7);
for i=1:7 
    [b(:,i),stats]=robustfit(FN(:,2),FN(:,i+2));
    stats=stats.se;
...
subplot(2,4,3:4);
% mmplotyy_temp([Fr(:,1)],[F_orig(:,end),FN(:,end)],'.',[F_orig(:,end-1),FN(:,end-1)],'rx'); % R6 & R5
ploty([Fr(:,1),F_orig(:,end),FN(:,end)],'.');
set(gca,'LineWidth',1);
% mmplotyy('R5');
ylabel('R6');
xlabel('day')
