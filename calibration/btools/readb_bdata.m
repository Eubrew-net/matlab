...
function [ozone,log,missing]=readb_bdata(brewer,path,brw_config_files,loc)
...
for ii=1:length(sname)
    ...
    if exist(bfile,'file')
       try
        ...
        lastwarn('');
        [ o3,config_,sl_,hg_,location]=readb_ds_develop(bfile,brw_config_files(i,1:2));
        %disp(['OK-> ',brw_name{i}]);
        catch
            err=lasterror;
        ...
        try
            if isempty(loc) || strmatch(loc,location_)
              dsum=[dsum;o3.dsum];
