...
datetick('keepticks');
axis([datenum(cal_year,cal_month,8),datenum(cal_year,cal_month,16),220,320])
box on
grid
ylabel('Dobson Units')
...
%        end
    ...
    suptitle('RBCC-E ')
    axis([datenum(cal_year,1,1)+dj-1+.25,datenum(cal_year,1,1)+dj-.20,200,300])
    legend(k,brw_name,-1);
 ...
 for i=1:length(brw)
     ...
     set(k(i),'color',c(i,:),'Marker',MK{i},'MarkerSize',4);
     if i==n_ref
        set(k(i),'color',[1,0,0],'Marker','d','LineStyle','-','LineWidth',3);
        %set(k(1),'color',[0,1,0],'Marker','d','LineStyle','-','LineWidth',2);
     end
     if i==n_inst
       set(k(i),'color',[0,0,0],'Marker','o','LineStyle','-','LineWidth',2);
 ...
 suptitle('RBCC-E ')
 axis([datenum(cal_year,1,1)+FINAL_DAYS(1),datenum(cal_year,1,1)+FINAL_DAYS(end),220,290])
    %legend(k,brw_name,-1);
    legend(k,[brw_name,([brw_name{n_inst},'_{orig}'])]);
