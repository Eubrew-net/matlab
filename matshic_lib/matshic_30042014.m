function matshic(datestart,dateend,site,inst,debu,pl)
%function matshic(datestart,dateend,site,inst,debu,pl)
...
%         pathout    % root path: <pathout>/yyyy/<inst>
...
if MATSHIC.fileformatout,
        matfilename=sprintf('%s/%04d/%s/matshic_%03d%04d.%s',ppout,yr,inst,doy,yr,inst);
end
if  debu,disp(sprintf('Processing %s : %d spectra',datestr(i),Nfiles));end
 ...
 mat.specraw{j}=spec;       
      ...
      [ETC,mat,HEADER]=prepareETC(spec,slit,et_spec,tm,sza,m,mat,HEADER,debu,j,dwll,P);  
        ...
        if ~exist(sprintf('%s/%04d/%s',ppout,yr,inst),'dir') ,
           ...
           if ~MATSHIC.fileformatout, mkdir(sprintf('%s/%04d/%s/shift',ppout,yr,inst));end
...
if ~MATSHIC.fileformatout, % 28 1 2014 JG old ascii format
 ...
 p_out=matfilename;
        ...
        dat=[wl Inom Iinst];
        dat(isnan(dat))=-999;          % 28 1 2014 remove nans and replace with -999
...
subplot(2,2,1);plot(wl,dwlfit,wl,buf);title('wavelength shifts');xlabel('Wavelength /nm');ylabel('Wavelength shift /nm');
grid;
...
buf(~ind)=nan;
plot(wl,buf);title(sprintf('Wavelength shift using new criteria [%.1f %.1f]',newqflag,newrescrit));grid;
xlabel('Wavelength /nm');ylabel('Wavelength shift /nm');


function [ETC,mat,HEADER]=prepareETC(spec,slit,et_spec,tm,sza,m,mat,HEADER,debu,j,dwll,P)
global MATSHIC

        indwl=spec(:,1)>310 & spec(:,1)<320;  % 13 4 2013
