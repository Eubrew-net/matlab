...
case 'start'
   if flagSel==0
      ...
      set(gcbf,'WindowButtonMotionFcn',' frames_s move');  
   	set(gcbf,'WindowButtonUpFcn',' frames_s stop');  
      frames_s move;
...
case 'stop'   
   ...
   case 'select'    
      if(flagSel==0)frames_s start;end;
   ...
   case 'r_select' % solo funciona en 2D 
      ...
      yi=find(uv.ss(:,Value)>y0 & uv.ss(:,Value)<y1); 
...
%         plot_uv_frame(uv,Value,flag3D);
   ...
   case 'del_select'
      spki=[ind_sel,Value*ones(size(ind_sel)),uv.time(ind_sel,Value),...
            uv.l(ind_sel,Value),uv.ss(ind_sel,Value)];
      uv.spikes=[uv.spikes;spki];   % añadimos el pico a spikes
      uv.ss(ind_sel,Value)=NaN;     % lo quitamos de uv
      h=findobj('Tag','B_del_select')
      set(h,'Enable','off');
   ...
   case 'save_mat'
      year=uv.date(1,1);
      dayj=uv.date(2,1);
      file_s=sprintf('ss%03d%02d',dayj,year)
      eval([' ',file_s,'=uv ;']);
      eval(['save ',file_s,' ',file_s]);
      write_interchange_ss(uv);
      h=findobj('Tag','B_save_mat');
      set(h,'Enable','off');
     
   case 'supr_scan'      
      if ~isempty(uv_trash)
         i_trash=(size(uv_trash.ss,2)+1);
      else i_trash=1;
      end   
   ...
   case 'prev'
    ...
    case  'next'
      % Value=get(gcbo,'Value');
       Value=Value+1;
       if Value>size(uv.ss,2) Value=size(uv.ss,2); end
       plot_uv_frame(uv,Value,flag3D)
   ...
   function plot_uv_frame(uv,Value,flag3D)
      ...
      if flag3D==1
       ...
       set(gca,'ButtonDownFcn','frames_s start');
   ...
   function plot_uv_sel(uv,Value,ind_sel,flag3D)
      hold on;
      if flag3D==1
        h=plot3(uv.l(ind_sel,Value),(uv.time(ind_sel,Value))/60,uv.ss(ind_sel,Value),'rx');
      else
        plot(uv.l(ind_sel,Value),uv.ss(ind_sel,Value),'+r');    
      end   
      hold off;        
