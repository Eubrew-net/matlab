...
date_initial=[datenum(2005,9,16) now];

% nlamp={'026','034','039','646','650','856','958','1080','1083'};% '1103','857','1202'
nlamp={'026','034','039','646','650','856','957','958','1004','1202','1080','1081','1083'};
%
% Año 2005: '646','650','957','1080','1081','1083'
% Año 2006: '646','650','957','958','1080','1081'
% Año 2007: '646','650','856','957','958','1080','1081','1083'
% Año 2008: '646','650','856','957','958','1004','1080','1081','1083'
% Año 2009: '026','034','039','646','650','856','958','1005','1202','1083'
% Año 2010: '856','1080'
% Todo:'026','034','039','646','650','856','957','958','1004','1202','1080','1081','1083'
%
...
dirin='.\QL185';  %directorio en el que se encuentran los ficheros QL de las lamparas a analizar
dirout='.\QL185\procesado'; %directorio al que se llevan las figuras creadas
dirirr='.\certificados'; %directorio en el que se encuentran los ficheros de irradiancia absoluta de las lámparas
s_ref='.\QL185\UVRES\uvr36309.185'; % respuesta usada como referencia
...
ref_5=r_50w(1,2:end);
H=mmplotyy(r_50w(:,1),median(matdiv(r_50w(:,2:end),ref_5)')*1.015,'o',temp_r50(:,2),':^',[10 40]);
...
hold on; 
plot(gca,r_50w(:,1),smooth(r_50w(:,1),median(matdiv(r_50w(:,2:end),ref_5)')*1.015,7),'-g^','Linewidth',2.5)
...
periodos={
%           periodo1: 17/Septiembre/2005 a 6/Octubre/2006 (between IOS's)
            [datenum(2005,9,17) datenum(2006,10,6)]
%           periodo2: 7/Octubre/2006 a 30/Julio/2008 (Despues de IOS06, hasta Aro2008)
            [datenum(2006,10,7) datenum(2008,07,30)]
%           periodo3: 31/Julio/2008 a 5/Septiembre/2009             
            [datenum(2008,07,31) datenum(2009,09,05)]
%           periodo4: 06/Septiembre/2009 a 27/Septiembre/2010 (between IOS's)
            [datenum(2009,09,06) datenum(2010,09,28)]
%           periodo5: 28/Septiembre/2010 (from IOS09 to now, last update: 28/June/2011)
            [datenum(2010,09,28) now]                     };
events_line=cell2mat(periodos)';
events_line=events_line(1:2:end); events_label={'IOS05','IOS06','Aro08','IOS09','IOS10'};  

% events_line=[datenum(2005,9,17),datenum(2006,10,6),...
%              datenum(2008,07,30),datenum(2009,09,05)]; events_label={'IOS05','IOS06','Arosa2008','IOS09'};  
...
for p=1:length(periodos)
    ...
    switch p
        case {1,3} % Interpolaciòn lineal en periodo 17/Septiembre/2005 a 6/Octubre/2006
                   % Idem                 en periodo 10/Julio/2008 a 5/Septiembre/2009
             for l=1:24
        ...
        case 2 % Interpolaciòn cuadrática en periodo 10/Octubre/2006 a 9/Julio/2008
             for l=1:24
                [poly_coeff(l,:),stat,mu(l,:)]=polyfit(r_kw(indx,1)',r_kw(indx,l+1)',2);
                intpol{p}(:,l+1)=polyval(poly_coeff(l,:),r_kw(indx,1)',[],mu(l,:));        % Sólo lamps
                extpol{p}(:,l+1)=polyval(poly_coeff(l,:),extpol_per,[],mu(l,:));           % Periodos
             end
             intpol{p}(:,1)=r_kw(indx,1);     extpol{p}(:,1)=extpol_per';
        case {4,5} % media de todas las lámparas para cada uno de los periodos
...
figure
H=plot(r_50w(:,1),median(matdiv(r_50w(:,2:end),ref_5)')*1.005,'o'); 
set(H,'LineWidth',2);
hold on; 
plot(gca,r_50w(:,1),smooth(r_50w(:,1),median(matdiv(r_50w(:,2:end),ref_5)'),7)*1.005,'-g^','Linewidth',2.5)
H=plot(r_kw(:,1),median(matdiv(r_kw(:,2:end),ref_)'),'--sk');
set(H,'LineWidth',2);
vl=vline(events_line,'-k',events_label); set(vl,'Linewidth',1.5); 
% set(vl(end),'Linewidth',1.5,'LineStyle','--'); 
...
vl=vline(events_line,'-r',events_label); set(vl,'Linewidth',1.5); 
...
title('Residuos (%)','FontWeight','Bold');
vl=vline(events_line,'-r'); set(vl,'Linewidth',1.5); 
datetick('x',12,'keeplimits','keepticks'); grid
