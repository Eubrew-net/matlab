% opciones_month.outputDir=fullfile('..','Lamps','QL185','procesado',sprintf('month%02d',month(now))); opciones_month.showCode=false;
% publish('ProcLampsCal_185_juanjo',opciones_month)
...
date_initial=datenum(2011,01,1);
nlamp={'026','034','039','856','1080','1083'};

nbr='185';      %numero del Brewer en estudio
dirin='..\Lamps\QL185';  %directorio en el que se encuentran los ficheros QL de las lamparas a analizar
% dirout='.\QL185\procesado'; %directorio al que se llevan las figuras creadas
dirirr='..\Lamps\certificados'; %directorio en el que se encuentran los ficheros de irradiancia absoluta de las lámparas

s_ref='..\Lamps\QL185\UVRES\UV2011HIST.txt'; 
fid=fopen(s_ref,'r');
ref=textscan(fid,'%s'); ref=ref{1};
s_ref=fullfile('..\Lamps\QL185\UVRES',ref{end}); % respuesta usada como referencia
...
for i=1:length(nlamp);
    if ~isempty(fichin{i})
       ...
       [ql_avg{i},ql_dat{i}]=load_ql_PLC(file_ql,dirirr,[],date_initial);
        ...
        ql_avg{i}.date=NaN; ql_avg{i}.temp=NaN;
    end    
...
ref_ind=find(diaj(r_50w(:,1))==138); ref_5=r_50w(ref_ind,2:end);
