...
function [uv,duv]=loaduvl_marta(path,response_struct,filter,uv)
...
%    if ~strcmpi(upper(file),upper(response))
       ...
       %[f,ty,IT,dt,CY,dark,time,JD,YR,lat,long,temp,filter]=loaduv_marta_def(file);
       [f,type,IT,dt,CY,dark,time,JD,YR,lat,long,temp,filter]=loaduv_marta_def(file);
       if upper(file(1))=='S' & isempty(f)
                    [f,ty,IT,dt,CY,dark,time,JD,YR,lat,long,temp,filter]=loaduv_ss(file);
       end
      
     if isstruct(response_struct) % si es una estructura (fichero_avg)
      % Elijo respuesta más cercana a al fichero UV que analizo
       year=r(2);
...
for i=1:length(dt)
    ...
    if exist ('response')==2 % si es un fichero
          resp=load(response);
  
     elseif isreal(response);                   % si es una matriz
        resp=response;           
    end
     try
       [uv,l]=irad([counts(:,1),counts(:,2)-slc_],resp);
         ...
         [duv_,duv_mkii,duv_o]=cost_duv([l',uv']);
          ...
          disp([ 'warning:', response, 'not found']);
          uv=NaN*zeros(size(l));  
      end
