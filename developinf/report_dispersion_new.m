function [tabla_dsp,dsp_quad,dsp_cubic]=report_dispersion_new(Cal,varargin)
...
SO2W=[0.00  -1.00   0.00 0.00 4.20 -3.20]; % so2 weithging factors
if length(arg.Results.date_range)==1
   [dsp_quad, dsp_cubic]=read_dsp_new(arg.Results.fpath,'brwid',Cal.brw_str{Cal.n_inst},...
...
aux(:,[1 2])=cat(2,dsp_quad(:,1),matadd(dsp_quad(:,17),-dsp_quad(:,16))); % calc step
%aux(:,3:14)=matadd(dsp_quad(:,4:15),-dsp_cubic(:,4:15)); % diference quad cubic
aux(:,3:14)=dsp_cubic(:,4:15); % diference quad cubic
aux(:,15:16)=cat(1,-O3W*abs(dsp_quad(:,18:23))',-O3W*abs(dsp_cubic(:,18:23))')';
aux(:,17:18)=cat(1,-O3W*abs(1e3*dsp_quad(:,24:29))',-O3W*abs(1e3*dsp_cubic(:,24:29))')';
aux(:,19:20)=cat(1,-O3W*abs(dsp_quad(:,36:41))',-O3W*abs(dsp_cubic(:,36:41))')';
aux(:,21:22)=cat(1,-SO2W*abs(dsp_quad(:,30:35))',-SO2W*abs(dsp_cubic(:,30:35))')';
...
%% Table, por periodos
 lbl_dsp={'Date','CSN','wl_0','wl_2','wl_3','wl_4','wl_5','wl_6',...
                'fwhm_0','fwhm_2','fwhm_3','fwhm_4','fwhm_5','fwhm_6',...
                'A1 quad.','std','A1 cubic','std','N','RayleighB','RayleighN','Aq_Bremen','Ac_Bremen','SO2_q','SO2_c'};
