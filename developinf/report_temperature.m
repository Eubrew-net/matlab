function [tabla_tc, sl_rw ,Fraw0,Fntc,Forig]=report_temperature(Cal,icf,varargin)
...
if ~exist(sprintf('sl_rw%03d.mat',Cal.brw(Cal.n_inst)),'file')
   sl_rw=readb_sl_rawl(fullfile(arg.Results.fpath,['bdata',Cal.brw_str{Cal.n_inst}],['B*.',Cal.brw_str{Cal.n_inst}]),...
     ...
     save(sprintf('sl_rw%03d.mat',Cal.brw(Cal.n_inst)),'sl_rw');
 else
      load(sprintf('sl_rw%03d.mat',Cal.brw(Cal.n_inst)));
 end
...
if any(id_period==0)
   ...
   date_range=arg.Results.date_range(y~=0);
   y(y==0)=[];
   id_period(id_period==0)=[]; 
else
...
for pp=1:length(id_period)        
    ...
    try       
     ...
     %[NTC1{pp},ajuste1{pp},Args,Fraw,Fnew,Forig]=temp_coeff_raw(config_temp,sl_rw,'outlier_flag',1,...
     %                                    'date_range',periods_([1,end]),'plots',0); 
                                     
     [NTC{pp},ajuste{pp},Argsx,Fraw0,Fntc,Forig]    =temp_coeff_raw(config_temp,sl_rw,'outlier_flag',1,'N_TC',icf_.data(2:6,pp)',...
                                         'date_range',periods_([1,end]),'plots',1,'temp_flag',[5,50]);
                                     
     %%
     %[NTC{pp},ajuste{pp},Argsx,Fraw0,Fntc]    =temp_coeff_raw(config_temp,sl_rw,'outlier_flag',1,'intensity_flag',1,...
     %                                     'date_range',datenum(2015,1,[30,100]),'temp_flag',[10,40],'plots',1);
     ...
     plot(Fntc(:,1),Fntc(:,end),'Color','m','Marker','.','LineStyle','None'); 
     
     ax1 = gca;  
     grid on; 
     title(sprintf('R6 Temperature dependence Brewer#: %s. Bfile temperature coeffs.\n%s to %s',...
            Cal.brw_str{Cal.n_inst},datestr(Forig(1,1),2),datestr(Forig(end,1),2))); 
     datetick('x',6,'KeepTicks','KeepLimits');
     pos=get(ax1,'Position');
     ax2 = axes;
     plot(Forig(:,1),Forig(:,2),'b.','MarkerSize',6); 
     hold on;
     
     ylabel('Temperature','Color','b');
     lg=legend(ax1,'Bfile temperature coeffs.'); 
     ...
     plot(Fntc(:,2),Fntc(:,end),'Color','m','Marker','*','LineStyle','None'); grid on; 
     
     yl=ylabel('Standard Lamp R6 ratio (summaries)','Units','Normalized'); xlabel('Temperature'); 
     [g h]=robust_line_no_legend;
     set(g(2),'LineWidth',2,'Color','r','Marker','None');
     set(findobj(gcf,'Type','Text'),'Visible','Off');
     lg=legend(g,sprintf('%.1f \\pm %.1f \\times T',round(h(2)*10)/10,round(h(1)*10)/10));
