%[x,r,ab,rp,data,osc_out,osc_smooth,outliers]=ratio_min_summary(a,b,n_min,name_a,name_b)
...
function [x,r,ab,rp,data,osc_out,osc_smooth,outliers]=ratio_min_summary(a,b,n_min,name_a,name_b)
% calcula el ratio entre respuestas o lamparas
MIN=60*24;

%a(find(a(:,2)<190 | a(:,2)>500),:)=NaN;
%b(find(b(:,2)<190 | b(:,2)>500),:)=NaN;
a=a(:,[1,10,3,2,8,9,4,5]);
b=b(:,[1,10,3,2,8,9,4,5]);
%n_min=10;
...
if nargin>3
   ...
   set(f,'Tag','RATIO_3'); 
    ...
    gscatter(sza,rp(:,2),{diaj(data(:,1)),j},[],'o+');
    
    xlabel('solar zenith angle');
    ylabel(' % ratio');
    title( [name_a,' - ',name_b,'/ ',name_b,' AM=+ PM=0'] )
    %legend('AM','PM');
    box on;
    grid;
    %figure;
    ...
    %figure by filter 
    f=figure;
    set(f,'Tag','RATIO_FILTER_INST');
    fbox=figure;
    set(fbox,'Tag','RATIO_FILTER_BOX');
    figure(f);
    if size(b,2)==8;   
    try
         gscatter(osc,rp(:,2),data(:,9)) %,data(:,end-data_l+1)],'','+o');
         set(gca,'Xlim',[250,1550]);
         xlabel('Ozone slant path');
         ylabel(' % ratio');
         title( [name_a,' - ',name_b,'/ ',name_b])
