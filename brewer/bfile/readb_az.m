...
% de la cabecer
function [sr,si,co]=readb_az(bfile,config)
    ...
    if ~isempty(jco)
        ...
        if ~isempty(jsr)
           for jj=1:length(jsr) 
            ...
            aux_sr=sscanf(lsr{3},'ze: Zenith discrepancy = %d ');
            aux_sr=[datefich(1)+aux_time(1)/24+aux_time(2)/60/24,datefich(2),datefich(3),aux_time',aux_sr];
            sr=[sr;aux_sr];
           end
        end
